...
只记录数量为1的幽灵样本，拾取事件与短周期库存差量共同覆盖地面拾取、合成及其他直接入包来源。
...
验证与执行。短冷却及短时窗口按玩家 UUID、能力 ID 和服务端绝对游戏时间隔离，默认不写入存档；退出
服务器时清理。内置“闪避”使用可重绑定的独立按键，客户端只提交前、后、左、右及四个对角线共八种有限方向；无方向
输入时默认向后。服务端验证玩家位于地面、未乘坐载具、未被眩晕且能力处于可用状态。成功后施加方向速度，
并向追踪该玩家的客户端广播本次真实水平位移向量。对角位移向量会归一化，避免组合方向获得额外速度。
客户端按真实位移向量与玩家当前身体朝向的夹角动态对齐翻滚轴，不再依赖固定方向动画，并使用短渐变平滑
进入和退出姿势。动画由必需前置 Player Animation Library 以高优先级控制器播放，
并在 8 Tick 动画窗口内按阶级降低 20%–90% 入站伤害，冷却 12 Tick。位移使用实体速度而非传送，因此仍
由原版碰撞处理阻挡；客户端不能直接决定位置、减伤或冷却结果。

`ability:charged_leap` 复用原版跳跃键和同一主动请求通道，但只有“潜行 + 跳跃”组合会被能力接管；
单独使用跳跃键始终保留原版跳跃、游泳与创造飞行行为。组合键按住时显示原版风格的跳跃蓄力条，
客户端在玩家逻辑执行前直接轮询物理跳跃键与潜行状态，避免原版潜行和按键映射的事件顺序吞掉组合键。
蓄满后保持 100% 而不回弹，松开跳跃键才释放跃斩，先松开潜行则取消。服务端重新校验地面、乘骑、
...
物品、权重和数量范围由各能力定义提供；因此嗅探奇物和招财猫的默认物品池可以被整合包完整替换。
