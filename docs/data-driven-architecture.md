...
`ability:charged_leap` 复用原版跳跃键和同一主动请求通道。按下空格后先进入 3 Tick 的短按判定窗口；
窗口内松开会立即执行普通跳跃，继续按住则显示原版风格的跳跃蓄力条，蓄满后保持 100% 而不回弹，
松开才释放跃斩。服务端重新校验地面、乘骑、游泳、攀爬、飞行、眩晕与能力状态；蓄力最多计入
