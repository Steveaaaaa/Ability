...
共八种有限方向，无方向输入时默认向前。若将闪避与潜行绑定到同一个实体按键，短按会在松开时翻滚，长按仍保持原版
...
客户端使用模组内置的关节模型和分段骨骼关键帧驱动胸部、头部、上臂、前臂、手、腿与小腿，前滚与后滚使用独立动作；
玩家皮肤、盔甲和手持物分别套用同一组关节姿势。动作先用 2 Tick 从当前姿态混入，再播放 13 Tick 主片段，服务端确认的
世界方向会先确定动作期间的模型朝向，再在模型局部坐标中以相反的旋转方向播放前滚或后滚。该实现不读取或加载第三方动画资源，因此 FantasyPower 不依赖 Player Animation
Library。默认 15 Tick 动作窗口内仍按阶级降低 20%–90% 入站伤害，冷却 15 Tick；总位移约 3.5 格，并按移动速度属性缩放。
服务器与本地客户端使用服务端同步的起始时间、方向、总距离和同一条根运动曲线进行带碰撞的实体移动，避免客户端的位置
上报覆盖服务端推进；服务端仍负责最终位置与规则判定，并在翻滚期间把碰撞体降至 0.6×0.8 格。客户端不能直接决定
减伤、持续时间或冷却结果。
