...
饥饿值。眩晕期间服务端暂停目标实体 Tick、清零移动并阻止其造成伤害；目标仍能受到其他来源伤害。
...
`ability:charged_leap` 复用同一主动请求通道的蓄力开始、蓄力释放和次要动作输入。客户端提供可重绑定的
“跃斩”键（默认 V），避免为了模拟原表格的空格蓄力而覆盖原版跳跃。服务端只允许活着、未旁观、未
眩晕且位于地面的玩家开始与释放蓄力；蓄力最多计入 20 Tick，垂直初速度在 0.42–0.74 之间插值，
最大值按原版运动近似对应 3.5 格高度。蓄力跳跃状态最长保留 100 Tick。

蓄力跳跃期间实际命中并造成生命伤害后，服务端先消耗跃斩状态，再以命中目标为中心搜索配置半径内
的生物，从而避免范围伤害递归触发自身。范围伤害读取玩家当前攻击力与阶级倍率，成功受伤的非友军
目标才会被眩晕；自己、同队实体及禁用 PvP 的玩家均被排除。命中同时获得一次只对摔落伤害生效的
短时保护。第 6 阶开始，蓄力跳跃状态允许在空中再次按跃斩键进行一次二段跳。
