...
客户端通过内置的分段骨骼关键帧驱动身体、头部、四肢和整体重心，前滚与后滚使用独立动作；盔甲、手持物和渲染层会随
玩家模型一起运动，动作方向由服务端确认的世界方向和玩家身体朝向共同计算。该实现不读取或加载第三方动画资源，因此
FantasyPower 不依赖 Player Animation Library。默认 13 Tick 动画窗口内仍按阶级降低 20%–90% 入站伤害，冷却 13 Tick；
总位移约 3.5 格，服务器使用与动画相同的分段根运动曲线推进，并在翻滚期间把服务端碰撞体降至 0.6×0.8 格。位移使用实体
