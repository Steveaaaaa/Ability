...
- 射击：反狙击 `counter_sniper`；远距离实际承伤标记攻击者，下一次有效伤害增幅并消耗标记；被锁定的袭击者身上会显示仅受击玩家可见、可穿透方块观察的像素瞄准标记，反击命中不追加特效。
- 射击：危险装药 `dangerous_charge`；弩射烟花爆炸伤害按阶级提高 60%–90%；触发能力的烟花会隐藏原版爆裂图案，改为由炽亮核心、破碎冲击环、放射火星与余烬烟云组成的分层像素爆炸。
- 射击：底火 `primer`；副手火焰弹右键蓄力后发射火球，并按阶级强化爆炸伤害；蓄力时火星沿物品模型向核心收缩，蓄满后白热脉动，发射产生点火环，强化火球带有白热核心、暗红外焰与像素余烬尾迹，命中时形成紧凑爆燃。
...
- 建造：环球旅行者 `world_traveler`；潜行并用空主手绑定具有物品能力的容器，背包侧栏提供9×4幽灵过滤槽；只有从世界中拾取的匹配物品会自动尝试存入绑定容器，背包和容器操作不触发路由，第二阶允许跨维度路由，并提供通用远程容器入口。
