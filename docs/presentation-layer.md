...
`assets/ability/textures/particle/crushing_blow_pressure.png`。强化后铁傀儡出现深色锻钢束带与铆钉；每次实际受伤都在受击面迸出橙黄至近白核心的锻造火星，并由手臂向肩部、胸甲依次点亮六段贴合模型 UV 的锻钢纹路。纹路不是悬浮标记；4–6 层阈值会按比例填满六段，新点亮段短暂全亮，满层时六段同时闪白。蓄力数值同时保留在强化者瞄准时的 HUD。满层时模型短暂下压，并以实际 7.5 格作用半径展开两道明亮的阶梯状像素地震环、低矮压力波和三层地表碎屑；受影响目标上方出现下压线，银色碎片随后由外圈回流至铁傀儡表示 5% 治疗，不使用绿色治疗粒子。若同一铁傀儡同时存在黑曜石加固，完整皮肤优先显示黑曜石侵蚀外观，贴体锻钢纹路与其余压碎效果仍正常显示。
