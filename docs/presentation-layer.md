...
“直取要害”在服务端每次增加破绽标记时发送持久 `ability:weak_point / ability:marks` 提示和一次 `mark_hit` 命中提示，提示中的等级值就是当前实际标记数。客户端为每层标记在目标碰撞箱的身体表面附着一道像素伤痕，不再绘制环绕目标的悬浮标志；剑、斧、长矛/三叉戟和钝器分别使用挥砍、劈砍、穿刺和钝击贴图。原版武器通过物品标签识别，模组武器可通过 NeoForge 通用标签 `c:tools/spears`、`c:tools/hammers`、`c:tools/maces` 参与适配，未声明对应标签的武器使用剑伤作为通用后备。达到阈值时服务端停止伤痕提示并发送 `trigger`：目标身体向命中方向喷射大量暗红血液粒子，客户端在周围实际地表逐渐生成像素血迹，保留约五秒后淡出。标记实例仍按攻击者、目标和标记 ID 隔离，避免多人攻击同一目标时互相覆盖。
