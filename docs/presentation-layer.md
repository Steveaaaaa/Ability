...
“紫颂果嬗变术”成功转换方块时，通过专用表现数据包同步原方块、目标方块、位置、阶级与是否消耗经验；客户端不依赖方块更新包的到达时序。原方块被拆分为 3×3×3 中除中心外的 26 个微型模型碎片，每块直接使用原方块的实际烘焙模型与纹理；碎片向外漂浮并在中段切换为目标材质，再向中心收束到已经转换完成的方块。外围符文框由断开的三段式像素条构成，八个角各有一枚方形能量块向中心汇聚；普通配方使用紫颂紫，高阶经验配方额外使用金色与黄绿色。整段约 1.1 秒，配合略微升调的原版紫颂果传送声，模组数据配方无需单独美术资源。
