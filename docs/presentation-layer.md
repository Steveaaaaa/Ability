...
“缠魔剑锋”由服务端周期性向玩家自身及追踪该玩家的客户端发送短时 `weapon_aura` 状态，只有能力已解锁且任一手持物具有足够的主手攻击伤害修正时才续期，因此能够自动兼容绝大多数模组近战武器。第一人称使用原版装备、挥动及长矛蓄力变换，第三人称直接挂接玩家模型手臂和对应手持物显示变换；三道错相旋转的淡紫色像素粒子环在物品模型局部空间中绘制，所以会随武器、手臂和攻击动作共同移动，并对主手与副手分别判断。
