...
“生还者”只在亡灵来源的减伤实际参与结算且玩家最终损失生命时发送 `shield_impact` 提示。命中方向由伤害来源位置投射到玩家碰撞体表面；客户端以玩家周围不可见的椭球护盾为曲面，在该方向生成持续约 0.75 秒的青蓝—淡紫色奥术涟漪。主环和分段副环沿真实曲面向外扩散，四枚像素化刻印随环旋转并淡出；护盾其余区域始终不可见，高阶只提高亮度并增加第三层细环。远程投射物使用伤害源位置确定涟漪方向，效果具有正常世界深度遮挡。
