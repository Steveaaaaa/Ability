...
`assets/ability/textures/particle/crushing_blow_pressure.png`。强化后铁傀儡出现深色锻钢束带与铆钉；每次实际受伤都在受击面爆出灰白金属火花。蓄力信息只显示在强化者瞄准时的 HUD，不在铁傀儡胸前悬浮图标或计数标记。满层时模型短暂下压，并以实际 7.5 格作用半径展开两道明亮的阶梯状像素地震环、低矮压力波和三层地表碎屑；受影响目标上方出现下压线，银色碎片随后由外圈回流至铁傀儡表示 5% 治疗，不使用绿色治疗粒子。若同一铁傀儡同时存在黑曜石加固，完整皮肤优先显示黑曜石侵蚀外观，其余压碎效果仍正常显示。
